...
LoRA SFT on the vision tower and merger (`ARM=vision+merger`) of bf16
`Qwen3.5-9B`, from 528 image-question-answer pairs. One stage per submission,
because they want different cards and each is a decision point:

```bash
STAGE=pool                 scripts/run_sft.sh   # case lists; not optional
STAGE=draft   sbatch ...   scripts/run_sft.sh   # teacher writes visual_evidence
STAGE=screen  sbatch ...   scripts/run_sft.sh   # can the STUDENT see it
STAGE=targets              scripts/run_sft.sh   # sft_wide.jsonl
STAGE=parity  sbatch ...   scripts/run_sft.sh   # token-id equality gate
STAGE=train   ARM=vision+merger EPOCHS=1 sbatch ... scripts/run_sft.sh
