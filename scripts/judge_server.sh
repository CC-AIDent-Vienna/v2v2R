...
# scripts/run_eval.sh as many times as you like against it; each run costs just
# the RadFact calls.
#
...
#   scripts/run_eval.sh outputs/<run>_validate
#
...
for i in $(seq 1 180); do          # 180 x 10s = 30 min
    ...
    if curl -s --max-time 5 "http://localhost:${PORT}/health" >/dev/null 2>&1; then
        ...
        echo "    scripts/run_eval.sh outputs/<run>_validate"
        echo ""
